...
export class AuthInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    if (this.isApiUrl(request.url)){
        ...
        if(this.authService.isTokenExpired(idToken) && !this.tokenRefreshInProgress){
      ...
      else if(this.authService.isTokenExpired(idToken) && this.tokenRefreshInProgress){
    ...
    if (idToken && this.isApiUrl(request.url)) {
      request = this.addTokenToRequest(idToken, request);    
      
      //console.log(this.authService.isTokenExpired(idToken));
